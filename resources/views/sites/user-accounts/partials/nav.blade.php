@php
    ...
    // ];
    try {
        $hasAds = \App\Models\Ad::query()->where('user_id', auth()->id())->exists();
    } catch (Throwable $e) {
        $hasAds = false;
    }
@endphp
...
<nav class="w-full md:w-64 shrink-0">
    <ul class="flex md:flex-col gap-2 md:gap-1">
        ...
        @if ($hasAds)
            ...
            <li>
                <a href="{{ route('user-ads.index')}}" class="block px-4 py-2 rounded-lg text-sm {{ $active ? 'bg-zinc-200 text-zinc-900' : 'text-zinc-600 hover:bg-zinc-50' }} hover:no-underline">

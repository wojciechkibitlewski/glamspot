    ...
    <div class="max-w-7xl mx-auto px-6 lg:px-8 mb-4 gap-4 my-8 flex flex-col md:flex-row">
        <div class="flex justify-center items-center w-full border border-gray-300 rounded-xl text-left mb-6 lg:mb-0">
            <div class="p-4 lg:m-[80px] w-full">
                ...
                <form method="POST" wire:submit="register" class="flex flex-col gap-6 mt-6 !font-light">
                    @csrf
                    <flux:input
                        wire:model="name"
                        :label="__('auth.name_field')"
                        type="text"
                        required
                        autocomplete="name"
                    ...
                    <flux:input
                        ...
                        :label="__('auth.password_confirmation_field')"
                        type="password"
                        required
                        autocomplete="new-password"
                        :placeholder="__('auth.password_confirmation_field')"
                        viewable
                    />

                    <div class="flex items-center justify-end">
                        <button data-test="register-button" type="submit" class="w-full px-8 py-4 rounded-full bg-gradient-to-r from-[#BA75EC] to-[#1FC2D7] text-white font-medium text-body-medium-m hover:opacity-90 transition cursor-pointer">
                            {{ __('auth.register') }}
                        </button>
                    ...
                    <div class="space-x-1 rtl:space-x-reverse text-left text-sm text-zinc-600 dark:text-zinc-400">
                        ...
                        <flux:link :href="route('login')" wire:navigate class="text-zinc-800 dark:text-zinc-900!">{{ __('auth.login') }}</flux:link>
                    </div>
                </form>

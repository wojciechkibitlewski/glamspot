...
<div class="w-full flex items-center justify-center bg-linear-to-l from-[#E446B4] to-[#6A80CE] py-8 mb-8">
    <div class="flex flex-row gap-4 w-full max-w-7xl mx-auto px-6 lg:px-8">
        <form method="GET" action="{{ route('ads.index') }}" class="flex flex-col md:flex-row w-full  bg-white p-0 md:p-3 md:px-6 md:pr-3 rounded-3xl md:rounded-full gap-6">
            ...
            <div x-data="citySearch('search', @js($currentCity ?? ''))" class="relative flex flex-row w-full max-w-[250px] gap-2 items-center px-4 md:p-0">
                <flux:icon.map-pin />
                <input
                    type="text"
                    name="city"
                    ...
                    class="w-full rounded-lg block text-base py-2 h-10 leading-[1.375rem] text-zinc-700 placeholder-zinc-400 bg-white p-4 pl-2 outline-transparent transition duration-300 ease focus:outline-none"
            ...
            <div class="flex items-center justify-end p-4 md:p-0">
                <button type="submit" class="w-full px-6 py-3 rounded-full bg-gradient-to-r from-[#BA75EC] to-[#1FC2D7] text-white font-medium text-body-medium-m hover:opacity-90 transition">
                    Szukaj
                </button>
            </div>
        </form>

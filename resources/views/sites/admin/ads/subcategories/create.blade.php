...
:title="__('Dodaj podkategorię - Panel Administratora')"
:description="__('Tworzenie nowej podkategorii ogłoszeń')"
>
    <div class="min-h-screen">
        <div class="flex items-center justify-between pb-4 mb-6 border-b border-zinc-200">
            <div>
                <flux:heading size="xl">Dodaj podkategorię</flux:heading>
                <flux:text class="mt-2 text-base text-zinc-600">Utwórz nową podkategorię ogłoszeń.</flux:text>
            </div>
            <flux:button href="{{ route('admin.ads.subcategories.index') }}" variant="ghost" icon="arrow-left">
                Powrót do listy podkategorii
            </flux:button>
        </div>
        ...
        <form action="{{ route('admin.ads.subcategories.store') }}" method="POST" class="space-y-6 max-w-3xl">
            @csrf
            ...
            <div class="bg-white rounded-lg border border-zinc-200 p-6 space-y-4">
                ...
                >
                    @foreach ($categories as $cat)
                        <option value="{{ $cat->id }}" @selected(old('category_id', $selectedCategoryId ?? null) == $cat->id)>
                            {{ $cat->name }}
                        </option>
                    @endforeach
                </select>

...
:title="__('Edytuj kategorię ogłoszeń - Panel Administratora')"
:description="__('Edycja kategorii ogłoszeń')"
>
    <div class="min-h-screen">
        <div class="flex items-center justify-between pb-4 mb-6 border-b border-zinc-200">
            <div>
                ...
                <flux:text class="mt-2 text-base text-zinc-600">{{ $category->name }}</flux:text>
            </div>
        ...
        <form action="{{ route('admin.ads.categories.update', $category) }}" method="POST" class="space-y-6 max-w-3xl">
            @csrf
            @method('PUT')
